...
import {
  ...
  ClipboardPenLine, Clock3, History, Megaphone, MessageCircle, MessageSquareText,
  PackageCheck, Pencil, Plus, Search, ShoppingBag, Sparkles, TrendingUp,
...
import { money, statusLabel, type OrderRecord, type Status } from './domain';
...
export function Dashboard() {
  ...
  return <DataScreen title={data.business.name}>
    ...
    <Card style={styles.metricCard}><View style={styles.metricLabel}><ClipboardPenLine color={colors.terracotta} size={22} /><Label color={colors.terracotta}>Promo Review Queue</Label></View><View><Text style={styles.cardTitle}>4 drafts need attention</Text><Text style={styles.smallCopy}>Next: Wednesday, 11:00 AM — Pork Adobo Bowl</Text></View></Card>
    ...
    <View style={styles.promoCandidate}><View style={styles.metricLabel}><Sparkles color={colors.amberDark} size={18} /><Text style={[styles.cardTitle, { color: colors.amberDark, fontSize: 14 }]}>Promotion Candidate</Text></View><Text style={styles.promoCopy}>{completed.length ? <><Text style={styles.bold}>{top}</Text> is suggested from completed online-order performance and current availability. Owner or authorized staff still selects the item before a draft is generated.</> : 'Complete an online order to see a suggested menu item here.'}</Text></View>
...
export function Menu() {
  ...
  const products = data?.products.filter((product) => product.name.toLowerCase().includes(query.trim().toLowerCase())) ?? [];
  return <DataScreen>
    <SearchBox value={query} onChangeText={setQuery} placeholder="Search menu" />
    {products.map((product) => { const allocation = data?.allocations.find((item) => item.product_id === product.id); const remaining = allocation ? allocation.total - allocation.used : 0; const percent = allocation?.total ? Math.max(0, Math.min(100, remaining / allocation.total * 100)) : 0; return <Card key={product.id} style={styles.menuCard}><View style={styles.photoWrap}><ProductPhoto path={product.photo_path} height={144} /><View style={styles.pricePill}><Text style={styles.priceText}>{money(product.price_centavos)}</Text></View></View><View style={styles.menuBody}><View><Text style={styles.cardTitle}>{product.name}</Text><Text numberOfLines={2} style={[styles.body, { fontSize: 13 }]}>{product.description || 'No description yet.'}</Text></View><View style={styles.allocation}><View style={styles.between}><View style={styles.flexOne}><View style={styles.inline}><PackageCheck color={colors.terracotta} size={15} /><Label>Online allocation</Label></View><Text style={styles.smallCopy}>{remaining} of {allocation?.total ?? 0} available today</Text></View><Text style={[styles.label, { color: product.active ? colors.terracotta : colors.muted }]}>{product.active ? 'Available' : 'Paused'}</Text></View><View style={styles.progress}><View style={[styles.progressFill, { width: `${percent}%`, backgroundColor: product.active ? colors.terracotta : '#D8C9C2' }]} /></View></View>{data?.role === 'owner' ? <AppButton compact label="Edit item" icon={<Pencil color={colors.terracotta} size={16} />} variant="ghost" onPress={() => router.push({ pathname: '/(owner)/menu-item/[id]', params: { id: product.id } })} /> : null}</View></Card>; })}
...
const styles = StyleSheet.create({
  ...
  timeline: { paddingLeft: 4, position: 'relative' }, rail: { backgroundColor: '#E5B9AD', bottom: 12, left: 20, position: 'absolute', top: 36, width: 2 }, day: { color: colors.muted, fontFamily: fonts.extraBold, fontSize: 14, marginBottom: 16, marginLeft: 50 }, timelineEntry: { flexDirection: 'row', gap: 12, marginBottom: 20 }, timelineIcon: { alignItems: 'center', backgroundColor: colors.terracottaSoft, borderColor: colors.cream, borderRadius: 22, borderWidth: 6, height: 44, justifyContent: 'center', width: 44, zIndex: 1 }, eventCard: { flex: 1 }, eventBadge: { backgroundColor: colors.amberSoft, borderRadius: radii.pill, maxWidth: 122, paddingHorizontal: 9, paddingVertical: 5 }, eventBadgeText: { color: colors.amberDark, fontFamily: fonts.semiBold, fontSize: 11, textTransform: 'capitalize' },
