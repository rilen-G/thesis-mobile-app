...
export function getStatusLabel(status: OrderStatus) {
  return status === 'Customer-Confirmed / Awaiting Staff Acceptance' ? 'Confirmed' : status;
}

export function getOrderBlockReason(order: Order) {
  if (order.status !== 'Customer-Confirmed / Awaiting Staff Acceptance') return null;
...
export function getOrderActions(order: Order): OrderAction[] {
  if (order.status === 'Customer-Confirmed / Awaiting Staff Acceptance') {
    return [
  ...
  if (order.status === 'Accepted' || order.status === 'Preparing') return [{ label: 'Ready', nextStatus: 'Ready', variant: 'primary' }];
